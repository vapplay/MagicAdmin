...
import { fetcher, formDataFetcher } from '@/ts/fetch';
...
// --- Interfaces (Ajustadas para coincidir con el backend) ---
...
const BannerEditModal = ({
  ...
  onClose: () => void;
  banner: Banner | null;
  allStories: HistoryModel[];
  createMutation: any;
  updateMutation: any;
}) => {
  // Usamos una interfaz interna para el formulario para manejar 'story'
  ...
  const handleSave = () => {
    ...
    if (formData.isPromo) {
      ...
      if (useExternalUrl && formData.externalUrl) {
        ...
        // Asegurarse de no enviar URL externa si no se usa
...
export default function BannersPage() {
  ...
  const { data: banners = [], isLoading: isLoadingBanners } = useQuery<Banner[]>({
    queryKey: ['banners'],
    queryFn: () => fetcher('banners'), // ¡Esta línea pide los datos de los banners!
  });

  console.log(banners);
  const { data: allStories = [], isLoading: isLoadingStories } = useQuery<HistoryModel[]>({
    queryKey: ['history'],
    queryFn: async () => {
      const response = await fetcher('history/all');
      return response.data as HistoryModel[];
    },
  });
  ...
  const handleOpenEditModal = (banner: Banner) => {
    // LOG 1: ¿Qué datos tiene el banner ANTES de abrir el modal?
    console.log(
      '[DEBUG Frontend] Abriendo modal para editar. Datos del banner:',
  ...
  if (isLoadingBanners || isLoadingStories)
    return (
